...
for i,num in pairs(called_numbers) do
	print("Start iter", i, num, #board_arr)
	continuing_boards = {}
	local remaining_board_count = #board_arr
	for board_id,board in pairs(board_arr) do
		if board.new_called_number(num) then
			if remaining_board_count <= 1 then
				winning_board = board
				winning_value = num
				print("Loser!")
				...
				remaining_board_count = remaining_board_count - 1
			end
		else
			table.insert(continuing_boards, board)
		end

	end
	if winning_board then break end
	board_arr = continuing_boards
end

print("Sum ", winning_board.unmarked_sum() * winning_value)
